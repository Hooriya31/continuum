...
🚧 In development. The Next.js scaffold, the `create-project` API route, and the Supabase schema (with tests) are in place. Not yet wired to a live Supabase project, so the create-project flow currently returns errors end-to-end — expected at this stage, not a bug. Next up: provisioning the real Supabase project/table and connecting it.
...
- [x] Next.js scaffold, project structure, and tooling
- [x] `create-project` API route + schema with unit tests
- [ ] Live Supabase project connected (auth + Postgres, RLS enabled)
- [ ] Free-text "save context" note before stopping work
